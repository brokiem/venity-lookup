...
export default function DuelsStatsViewer({ gamertag, gamemode }: { gamertag: string, gamemode: string }) {
    const [stats, setStats] = useState(null as any);

    useEffect(() => {
        fetch(`https://api.venitymc.com/player/${gamertag}/duels`).then(r => r.json()).then(r => {
            setStats(r);
        });
    }, []);
