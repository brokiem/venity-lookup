...
export default function SkyWarsStatsViewer({ gamertag, gamemode }: { gamertag: string, gamemode: string }) {
    const [stats, setStats] = useState(null as any);

    useEffect(() => {
        fetch(`https://api.venitymc.com/player/${gamertag}/skywars`).then(r => r.json()).then(r => {
            setStats(r);
        });
    }, []);
